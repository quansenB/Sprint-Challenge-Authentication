...
Sessions are used to store user data, because Http otherwise is stateless and can not transition state data over time.
1. What does bcrypt do to help us store passwords in a secure manner.
bcrypt uses hashing with a salt to make brute force attacks significantly harder. It also sues a slow hasing algorythm multiple times to make the passwords even more secure. 
1. What does bcrypt do to slow down attackers?
It repeats the hasing algorithm and uses a verry slow one. It also sues a user specific salt to prevent attacks to multiple users with the same password.
